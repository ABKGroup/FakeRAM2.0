### BELOW is the edited version of the file copied from bsg_fakeram
## Updates :
#  Removed "flip" to allow pins only on a single layer
#  Added pin offset == pin_pitch
#  Added changes made by James Cherry
#  Changed obstruction shapes
...
def create_lef( mem ):
    ...
    metalLayer      = mem.process.metalLayer
    ...
    #group_pitch = math.floor((number_of_tracks_available - number_of_pins*track_count) / 4)*mem.process.pinPitch_um #what is this / 4 ?
    ...
    y_step = y_offset - (y_offset % manufacturing_grid_um) + (mem.process.pinWidth_um/2.0)
    ...
    # No pins (full rect)  
    pin_layer_number = metalLayer.replace(metalPrefix, "", 1)
    ...
    fid.write('    LAYER %s%d ;\n' % (metalPrefix,int(pin_layer_number)))
